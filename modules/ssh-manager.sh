...
# Module metadata
module_info() {
    echo "SSH Manager|Configure SSH server, MOTD, and prompt colors"
}
...
show_status() {
    ...
    if [[ -f "$BASH_PROMPT_FILE" ]]; then
        info+="Custom prompt: Installed\n"
    else
        info+="Custom prompt: Not installed\n"
    fi
...
# Generate ASCII art for hostname
generate_ascii_hostname() {
    local hostname="$1"
    local font="${2:-small}"

    # Use figlet if available, otherwise create simple banner
    if command_exists figlet; then
        figlet -f "$font" "$hostname" 2>/dev/null || echo "$hostname"
    elif command_exists toilet; then
        toilet -f small "$hostname" 2>/dev/null || echo "$hostname"
    else
        # Simple ASCII banner
        local len=${#hostname}
...
module_main() {
    while true; do
        ...
        choice=$(ui_menu "SSH Manager" "Select operation:" \
            ...
            "motd-show" "Show current MOTD" \
            "motd" "Install MOTD banner" \
            "motd-remove" "Remove MOTD banner" \
        ...
        case "$choice" in
            status)        show_status ;;
            motd-show)     show_current_motd ;;
            motd)          install_motd_banner ;;
            motd-remove)   remove_motd_banner ;;
