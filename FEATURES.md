...
Install and manage LXD/LXC containers with full lifecycle management.

- **Installation** - Install LXD via snap with automatic initialization
- **Initialization** - Choose between automatic (recommended) or manual configuration
  - Automatic mode: Creates bridge network with IPv4/IPv6 NAT and default storage pool
  - Manual mode: Interactive configuration wizard
- **Status Display** - Show LXD version, service status, resource counts, and disk usage
- **Network Management**
  - List all networks and view detailed configuration
  - Create bridge networks with IPv4/IPv6 NAT (default, recommended)
  - Create macvlan networks for direct physical network access
  - Configure custom subnets and gateways
  - Remove unused networks
- **Container Lifecycle Management**
  - Create containers from multiple distributions (Ubuntu, Debian, Alpine)
  - List all containers with status and IP addresses
  - Start, stop, and restart containers
  - Delete containers with safety confirmations
  - Execute shells in running containers (bash, sh, custom)
  - View detailed container information and logs
- **Web UI Management**
  - Enable and configure LXD's built-in web interface
  - Choose listening address: localhost (127.0.0.1) or all interfaces (0.0.0.0)
  - Configure custom port (default: 8443)
  - Set trust password for authentication
  - Security warnings for network-exposed configurations
- **Storage Management**
  - List storage pools
  - Create storage pools with multiple drivers (dir, zfs, btrfs, lvm)
  - View pool details and usage
  - Delete unused pools
- **Profile Management**
  - List and view profile configurations
  - Show profile YAML configuration
  - Copy profiles for customization
- **Snapshots**
  - Create container snapshots for backup
  - List snapshots for containers
...
- **UFW Integration**
  ...
  - Configure DHCP/DNS traffic rules
- **Service Management**
  - Start, stop, and restart LXD service
  - View service status
- **Uninstallation**
  - Remove LXD with optional data cleanup
